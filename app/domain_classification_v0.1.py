...
# @Author :llj
'''训练domain领域分类模型
基本思路：用DL文本分类任务+规则的方法
 创建 寒暄库 常用词典，行业库常用词典，12366库常用词典'''
import pymysql
import numpy as np
import pandas as pd
import random
...
def split_data():#加载数据，分割训练数据 & 测试数据
    ...
    train_x,train_y=np.array(dp.sentence2idx(train_texts)),dp.label2onehot(train_labels)
    test_x,test_y=np.array(dp.sentence2idx(test_texts)),dp.label2onehot(test_labels)
...
class CNN_Model():
    ...
    def train(self,model,train_x, train_y):#传入的是处理好的词典id序列
        ...
        model.fit(train_x, train_y,epochs=self.hyper_parameters['model']['epochs'],verbose=1,batch_size=self.hyper_parameters['model']['batch_size'],callbacks=[early_stopping])
        self.presist(model)
        pass
    ...
    def text_vector(self,model,text1):#将文本表示成向量形式
        ...
        p=text_vector_model.predict(x=text1_index)
...
def train():
    ...
    cnn=CNN_Model(hyper_parameters)
